...
class EventHandler(Handler):
    ...
    def find_event_choice(self, state: GameState) -> str:
        ...
        if event_name == "Big Fish":  # Banana, Donut, Box Event
        ...
        if event_name == "The Cleric":
            if 'purify' in state.get_choice_list():
                return "choose purify"  # Remove card
            if hp_per <= 30 and 'heal' in state.get_choice_list():
                return "choose heal"
            # if hp_per >= 90:
        ...
        if event_name == "Golden Idol":
            if state.has_relic("Ectoplasm"):  # probably shouldnt happen with this strat
